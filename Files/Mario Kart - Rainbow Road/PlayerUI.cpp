...
bool PlayerUI::IsPlayerInFirst() const
{
	...
	VehicleMove* em = mOwner->mGame->mEnemy->GetComponent<VehicleMove>();
	if (pm->currentLap > em->currentLap) {
		return true;
	}
	else if(pm->currentLap == em->currentLap){
		if (pm->lastCheck > em->lastCheck) {
			return true;
		}
		else if (pm->lastCheck == em->lastCheck) {
			if (pm->dToNextCheck() < em->dToNextCheck()) {
				return true;
			}
		}
	}
